...
export type ToolPresetMap = Record<ToolId, string | null>
...
export type EditorLogic = {
  ...
  setPreset(tool: ToolId, presetId: string | null): EditorLogicResult
...
function isEditorLogic(value: unknown): value is EditorLogic {
  ...
  return (
    ...
    typeof Reflect.get(value, 'setPreset') === 'function' &&
...
type HistoryEntry = { intensity: number; selectedTool: ToolId | null; presets: ToolPresetMap }

function createDefaultPresets(): ToolPresetMap {
  return { aiFilters: null, adjust: null, text: null, overlays: null }
}

function clonePresets(presets: ToolPresetMap): ToolPresetMap {
  return { ...presets }
}
...
function createDefaultLogic(get: () => EditorStoreState, set: (fn: (s: EditorStoreState) => EditorStoreState) => void): EditorLogic {
  return {
    selectTool(tool: ToolId): EditorLogicResult {
      set((s) => {
        const next: HistoryEntry = { intensity: s.tool.intensity, selectedTool: tool, presets: clonePresets(s.tool.presets) }
        const past = [...s.history.past, { intensity: s.tool.intensity, selectedTool: s.tool.selectedTool, presets: clonePresets(s.tool.presets) }]
        return {
          ...s,
    ...
    setIntensity(value: number): EditorLogicResult {
      ...
      set((s) => {
        const past = [...s.history.past, { intensity: s.tool.intensity, selectedTool: s.tool.selectedTool, presets: clonePresets(s.tool.presets) }]
        const present: HistoryEntry = { intensity, selectedTool: s.tool.selectedTool, presets: clonePresets(s.tool.presets) }
        return {
          ...s,
    ...
    setPreset(tool: ToolId, presetId: string | null): EditorLogicResult {
      set((s) => {
        const presets = { ...s.tool.presets, [tool]: presetId }
        const past = [...s.history.past, { intensity: s.tool.intensity, selectedTool: s.tool.selectedTool, presets: clonePresets(s.tool.presets) }]
        const present: HistoryEntry = { intensity: s.tool.intensity, selectedTool: s.tool.selectedTool, presets: clonePresets(presets) }
        return {
          ...s,
          tool: { ...s.tool, presets },
          history: { past, present, future: [] },
    ...
    undo(): EditorLogicResult {
      ...
      const future = [{ intensity: state.tool.intensity, selectedTool: state.tool.selectedTool, presets: clonePresets(state.tool.presets) }, ...state.history.future]
      set((s) => ({
        ...s,
        tool: { selectedTool: previous.selectedTool, intensity: previous.intensity, presets: clonePresets(previous.presets) },
    ...
    redo(): EditorLogicResult {
      ...
      const past = [...state.history.past, { intensity: state.tool.intensity, selectedTool: state.tool.selectedTool, presets: clonePresets(state.tool.presets) }]
      set((s) => ({
        ...s,
        tool: { selectedTool: next.selectedTool, intensity: next.intensity, presets: clonePresets(next.presets) },
...
export type EditorStoreState = {
  ...
  actions: {
    ...
    setPreset(tool: ToolId, presetId: string | null): void
    undo(): void
    redo(): void
    export(): Promise<void>
  }
...
export const useEditorStore = create<EditorStoreState>((set, get) => {
  ...
  return {
    ...
    tool: { selectedTool: null, intensity: 50, presets: createDefaultPresets() },
    undoRedo: { canUndo: false, canRedo: false },
    history: { past: [], present: { intensity: 50, selectedTool: null, presets: createDefaultPresets() }, future: [] },
    ...
    actions: {
      ...
      setPreset(tool: ToolId, presetId: string | null): void {
        const res = get().logic.setPreset(tool, presetId)
        if (!res.ok) logger.error('setPreset failed', { error: res.error })
      },
      undo(): void {
